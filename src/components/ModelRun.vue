    ...
    <v-row>
      <h2>Model Run: <span id="model_run_name" :contenteditable="model_run_editable" @blur="update_title_and_description">{{ waterspout_data.name }}</span>
        <v-icon v-if="model_run_editable"
            class="sc_edit_icon"
            @click="start_editing_element('model_run_name')">mdi-pencil</v-icon></h2>
    </v-row>
    <v-row>
  ...
  data: function() {
      return {
          ...
          crop_modifications_headers: [
            ...
            {title: 'Price Proportion', key: 'price_proportion' },
            {title: 'Yield Proportion', key: 'yield_proportion' },
            {title: 'Min Land Area Proportion', key: 'min_land_area_proportion' },
            {title: 'Max Land Area Proportion', key: 'max_land_area_proportion' },
